...
class SystemEvent(models.Model):
    ...
    device = models.ForeignKey(
        Device, on_delete=models.CASCADE, related_name="system_events"
    )
...
class EventLog(models.Model):
    ...
    event = models.ForeignKey(
        SystemEvent, on_delete=models.CASCADE, related_name="logs"
    )
    action = models.CharField(max_length=255, help_text="Action taken or event detail")
    details = models.JSONField(
        default=dict, blank=True, help_text="Full context and metadata"
    )
...
class AlertRule(models.Model):
    ...
    severity = models.CharField(
        max_length=20, choices=SystemEvent.SEVERITY_CHOICES, default="medium"
    )
    is_active = models.BooleanField(default=True)
    notification_channels = models.JSONField(
        default=list, help_text="Email, SMS, Slack, webhook URLs"
    )
    escalation_rules = models.JSONField(
        default=dict, blank=True, help_text="Escalation configuration"
    )
    cooldown_minutes = models.PositiveIntegerField(
        default=60, help_text="Minimum time between alerts"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def __str__(self):
        target = (
            ...
            if self.device
            else (self.asset.asset_id if self.asset else "Global")
        )
...
class NotificationQueue(models.Model):
    ...
    alert_rule = models.ForeignKey(
        AlertRule, on_delete=models.CASCADE, related_name="notifications"
    )
    event = models.ForeignKey(
        SystemEvent, on_delete=models.CASCADE, related_name="notifications"
    )
