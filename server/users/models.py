...
class UserDeviceRole(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='device_roles')
    device = models.ForeignKey(Device, on_delete=models.CASCADE, related_name='user_roles', null=True, blank=True)
    site = models.ForeignKey(Site, on_delete=models.CASCADE, related_name='user_roles', null=True, blank=True)
    role = models.CharField(max_length=20, choices=ROLE_CHOICES, default='viewer')
    permissions = models.JSONField(default=dict, blank=True, help_text="Specific permissions override")
    ...
    def __str__(self):
        target = self.device.serial_number if self.device else (self.site.name if self.site else 'Global')
